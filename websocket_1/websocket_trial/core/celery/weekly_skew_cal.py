...
dte_nfy = daily_morn_df['dte_weekly_nfy'].item()
dte_bnk = daily_morn_df['dte_weekly_bnk'].item()
...
def calc_ab_weekly(symbol):
    ...
    # while True:
    # k = weekly_expiry_live.objects.filter(symbol=symbol, instrument_type='FUT').last()
    ...
    monthly_fut_close = k.close
    monthly_atm_strike = round(monthly_fut_close / strike_diff) * strike_diff

    monthly_strike_ce = weekly_expiry_live.objects.filter(Q(symbol=symbol) & Q(strike_price=monthly_atm_strike) &
                                                        (Q(instrument_type='CE'))).last()
    monthly_strike_pe = weekly_expiry_live.objects.filter(Q(symbol=symbol) & Q(strike_price=monthly_atm_strike) &
                                                        (Q(instrument_type='PE'))).last()
    fut_close = monthly_atm_strike + monthly_strike_ce.close - monthly_strike_pe.close
    ...
    latest_concerned = weekly_expiry_live.objects.filter(Q(symbol=symbol) & Q(strike_price__gt=lower_threshold) &
    ...
    try:
        ...
        put_b_value = (put_strike1['iv1'].item() * (put_skew_atm_diff2) ** 2 - put_strike2['iv1'].item() * (
            put_skew_atm_diff1) ** 2) / (
                                  (put_skew_atm_diff2 - put_skew_atm_diff1) * put_skew_atm_diff1 * put_skew_atm_diff2)
        # print(put_b_value)
        ...
        put_a_value = (put_strike1['iv1'].item() * put_skew_atm_diff2 - put_strike2[
            'iv1'].item() * put_skew_atm_diff1) / (
                                  (put_skew_atm_diff1 - put_skew_atm_diff2) * put_skew_atm_diff1 * put_skew_atm_diff2)
    ...
    try:
        ...
        call_b_value = (call_strike1['iv1'].item() * (call_skew_atm_diff2) ** 2 - call_strike2['iv1'].item() * (
            call_skew_atm_diff1) ** 2) / ((
                                                      call_skew_atm_diff1 - call_skew_atm_diff2) * call_skew_atm_diff1 * call_skew_atm_diff2)
        # print(put_b_value)
        ...
        call_a_value = (call_strike1['iv1'].item() * call_skew_atm_diff2 - call_strike2[
            'iv1'].item() * call_skew_atm_diff1) / ((
                                                                call_skew_atm_diff1 - call_skew_atm_diff2) * call_skew_atm_diff1 * call_skew_atm_diff2)
    ...
    objects_to_update = weekly_skew_table.objects.filter(symbol=symbol)
    ...
    if not objects_to_update.exists():
        ...
        inserts.append(weekly_skew_table(**temp))
        weekly_skew_table.objects.bulk_create(inserts, batch_size=500)
