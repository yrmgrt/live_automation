...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        (frame8_0, frame8_1, frame8_2, frame8_3, frame8_4, frame8_5, frame8_6) = tuple(frames[83:90])
        ...
        my_frames = my_frames + [frame8_0, frame8_1, frame8_2, frame8_3, frame8_4, frame8_5, frame8_6]
        ...
        coord_place = coord_place + [[2,2,28,1,centre_orient,(2,0),(2,0)]]
        coord_place = coord_place + [[2,2,29+i,1,centre_orient,0,(2,0)] for i in range(6)]
        ...
        columns8_0 = ('%')
        columns8_1 = ('intra_long', 'iv')
        columns8_2 = ('fwd_iv',)
        columns8_3 = ('hv',)
        columns8_4 = ('fair_iv',)
        columns8_5 = ('IVP',)
        columns8_6 = ('days_theta',)
        ...
        table_columns = table_columns + [columns8_0, columns8_1, columns8_2, columns8_3, columns8_4, columns8_5, columns8_6]

        table_heights = []
        for i in range(20):
            table_heights.append(14)
        for i in range(16):
            table_heights.append(30)
        for i in range(15):
            table_heights.append(30)
        for i in range(14):
            table_heights.append(14)
        ...
        (table8_0, table8_1, table8_2, table8_3, table8_4, table8_5, table8_6) = tuple(tree_tables[83:90])
        ...
        my_tables = my_tables + [table8_0, table8_1, table8_2, table8_3, table8_4, table8_5, table8_6]
        ...
        while True:
            ...
            for i, row in table_df8.iterrows():
                vals0 = str(round(row['pct_change'],1))
                vals1 = (row['symbol'], round(row['current_iv'],1))
                vals2 = (round(row['fwd_iv'],1))
                vals3 = (round(row['hv_current'],1))
                ...
                condition1_2 = (row['current_iv'] < row['fwd_iv'])
                if datetime.datetime.now() < today_date_10_30:
                    condition1_3 = row['days_theta'] >= 0
                else:
                    condition1_3 = row['days_theta'] >= 0.5
                condition1_4 = row['hv_current'] >= 0.9*row['current_iv']
                condition1_5 = row['long_moves_yest'] >= 3

                condition2_1 = (60 < row['ivp'] <= 90)
                if datetime.datetime.now() < today_date_10_30:
                    condition2_2 = (row['current_iv'] < row['fwd_iv'])
                    condition2_3 = row['days_theta'] >= 0
                else:
                    condition2_2 = (row['current_iv'] < (row['fwd_iv'] + row['bench_mark_iv'])/2)
                    condition2_3 = row['days_theta'] >= 0.5
                condition2_4 = row['hv_current'] >= 0.9*row['current_iv']
                condition2_5 = row['long_moves_yest'] >= 3

                condition3_1 = (90 < row['ivp'])
                condition3_2 = (row['current_iv'] < row['bench_mark_iv'])
                if datetime.datetime.now() < today_date_10_30:
                    condition3_3 = row['days_theta'] >= 0
                else:
                    condition3_3 = row['days_theta'] >= 0.5
                condition3_4 = row['hv_current'] >= 0.9*row['current_iv']
                condition3_5 = row['long_moves_yest'] >= 3
                ...
                if (condition1_1 and condition1_2 and condition1_3 and condition1_4 and condition1_5)\
                    ...
                    if row['fwd_iv'] == 0:
                        continue
                    if row['days_theta'] >= 0.5 and row['pct_change'] < 0:
                        table8_0.insert('', 'end', values=vals0, tags=('%_neg_highlight'))
                    elif row['days_theta'] >= 0.5 and row['pct_change'] > 0:
                        table8_0.insert('', 'end', values=vals0, tags=('%_pos_highlight'))
                    else:
                        table8_0.insert('', 'end', values=vals0)
                    if row.symbol in future_results:
                        table8_1.insert('', 'end', values=vals1, tags=('future_result'))
                    elif row.symbol in past_results:
                        table8_1.insert('', 'end', values=vals1, tags=('past_result'))
                    else:
                        table8_1.insert('', 'end', values=vals1)

                    if row['ivp'] >= 60:
                        table8_5.insert('', 'end', values=vals5, tags=('short_highlight'))
                    elif row['ivp'] <= 40:
                        table8_5.insert('', 'end', values=vals5, tags=('long_highlight'))
                    else:
                        table8_5.insert('', 'end', values=vals5)

                    if row['current_iv'] >= 1.2*row['fair_vol']:
                        table8_4.insert('', 'end', values=vals4, tags=('short_highlight'))
                    elif row['current_iv'] <= 0.8*row['fair_vol']:
                        table8_4.insert('', 'end', values=vals4, tags=('long_highlight'))
                    else:
                        table8_4.insert('', 'end', values=vals4)

                    if row['current_iv'] >= 1.2*row['fwd_iv']:
                        table8_2.insert('', 'end', values=vals2, tags=('short_highlight'))
                    elif row['current_iv'] <= 0.8*row['fwd_iv']:
                        table8_2.insert('', 'end', values=vals2, tags=('long_highlight'))
                    else:
                        table8_2.insert('', 'end', values=vals2)

                    if row['current_iv'] >= 1.2*row['hv_current']:
                        table8_3.insert('', 'end', values=vals3, tags=('short_highlight'))
                    elif row['current_iv'] <= 0.8*row['hv_current']:
                        table8_3.insert('', 'end', values=vals3, tags=('long_highlight'))
                    else:
                        table8_3.insert('', 'end', values=vals3)

                    if row['days_theta'] < 1:
                        table8_6.insert('', 'end', values=vals6, tags=('short_highlight'))
                    else:
                        table8_6.insert('', 'end', values=vals6, tags=('long_highlight'))
