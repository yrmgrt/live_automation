            ...
            for row in table_values:
                ...
                if not sym in ['NIFTY', 'BANKNIFTY']:
                    ...
                    if latest_result_date <= '2023-08-15' and latest_result_date >= '2023-08-01':
                        continue
                filtered = vol_table.objects.filter(symbol=sym).last()
                ...
                sym_corr_df = corr_df[corr_df['symbol'] == sym]
                ...
                if (row.call_b_value < call_front_benchmark) and (row.call_strikes_ratio >= 0.7)\
                    ...
                    temp['entry'] = round(sym_df.call_std_down_straddle_skew.item()/0.01)*0.01
                    temp['exit'] = round(sym_df.call_std_up_straddle_skew.item()/0.01)*0.01
                    ...
                    temp['correlation_last_day'] = round(sym_corr_df.last_day_correlation.item()/0.01)*0.01
                    temp['correlation_last_week'] = round(sym_corr_df.last_week_correlation.item() / 0.01) * 0.01
                ...
                elif (row.put_call_b_value < put_call_bench_mark) and (row.put_call_strikes_ratio >= 0.7)\
                ...
                elif (row.call_b_value > call_call_bench_mark) and (row.call_strikes_ratio >= 0.7)\
                        and (row.R2_call >= 0.75):
                    temp = dict()
                    ...
                    inserts.append(scanner_table_skew(**temp))
                    scanner_table_skew.objects.bulk_create(inserts, batch_size=500)
                elif (row.put_call_a_value < put_call_a_benchmark) and(row.R2_put_call >= 0.8):
                    temp = dict()
                    ...
                    temp['exit'] = round(sym_df.put_call_std_up_straddle_kurt,2)
